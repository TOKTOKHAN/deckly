...
export default function CreateUserModal({ isOpen, onClose, onSuccess }: CreateUserModalProps) {
  ...
  const validateForm = (): boolean => {
    const errors = validateUserForm({ name, email, password, phone }, { passwordRequired: true });
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validationErrors.password) {
      const error = validateField('password', value, { passwordRequired: true });
      setValidationErrors(prev => ({ ...prev, password: error }));
    }
  };

  const handlePasswordBlur = () => {
    const error = validateField('password', password, { passwordRequired: true });
    setValidationErrors(prev => ({ ...prev, password: error }));
  };
