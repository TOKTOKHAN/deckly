...
interface ProposalDashboardSkeletonProps {
  /** 외부 컨테이너를 포함할지 여부 (default: true) */
  includeWrapper?: boolean;
}

export default function ProposalDashboardSkeleton({
  includeWrapper = true,
}: ProposalDashboardSkeletonProps) {
  const content = (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-8 flex min-h-[56px] items-center justify-between">
      ...
      <div className="grid min-h-[400px] grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {[1, 2, 3, 4, 5, 6].map(i => (
          <div
            key={i}
            className="group min-h-[200px] cursor-pointer rounded-3xl border border-gray-100 bg-white p-6 shadow-sm"
          >
